...
def make_state(my_pos, snack_pos, other_snake_pos, dirnx, dirny, self_body, other_snake_body):
  ...
  # Categorize relative position into bins
  if snack_dx < 0:
    x_dir = 0
  elif snack_dx == 0:
    x_dir = 1
...
class Super_Snake:
    ...
    def calc_reward(self, snack, other_snake):
        ...
        if(self.head.pos[1] < 1):
            up_wall = 1
        if(right_wall and self.dirnx == 1):
            reward -= 100
        elif(right_wall and self.dirnx != 1):
            pass
        
        if(left_wall and self.dirnx == -1):
            reward -= 100
        elif(left_wall and self.dirnx != -1):
            pass

        if(up_wall and self.dirny == -1):
            reward -= 100
        elif(up_wall and self.dirny != -1):
            pass

        if(down_wall and self.dirny == 1):
            reward -= 100
        elif(down_wall and self.dirny != 1):
            pass
